...
如果后续要做 Kubernetes 兼容性评估，优先关注的也是这几个资源族：

- `apiextensions.k8s.io/v1` CRD
- `admissionregistration.k8s.io/v1` webhook
- `batch/v1` Job
- `core/v1` ConfigMap / Secret / ServiceAccount / Service
- `rbac.authorization.k8s.io/v1` RBAC
- 外部依赖 CRD：`extensions.kubesphere.io/v1alpha1` `JSBundle`
